...
FUNCTION CRISPEX_BGROUP_STOKES_SELECT_SP, event, NO_DRAW=no_draw
  WIDGET_CONTROL, event.TOP, GET_UVALUE = info
    ...
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_BGROUP_STOKES_SELECT_SP'
...
FUNCTION CRISPEX_BGROUP_STOKES_SELECT_XY, event, NO_DRAW=no_draw;, SET_STOKES=set_stokes
  WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_BGROUP_STOKES_SELECT_XY'
...
; Sets the zoomfactor and changes options and paramters accordingly
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_BGROUP_ZOOMFAC_SET'
...
; Handles the change mask overlay window
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_BGROUP_MASK_OVERLAY'
...
; Cursor handling procedure, tracks and handles events from the cursor on the main image
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_CURSOR', /IGNORE_LAST
...
; Handles selection of diagnostics and calls necessary replot procedures
  ...
  IF ((*(*info).intparams).ndisp_diagnostics NE (*(*info).intparams).ndisp_refdiagnostics) THEN BEGIN
    IF (*(*info).ctrlsswitch).lp_ref_lock THEN $
      CRISPEX_SLIDER_LP_REF_LOCK, event, /UNLOCK
...
; Sets up the parameter overview window
    ...
    IF ((*(*info).plotswitch).heightset EQ 0) THEN BEGIN
          ...
          VALUE = STRTRIM(STRING((*(*info).plotaxes).v_dop[(*(*info).dataparams).lp],$
    ...
    ; Column 3 (if reference present) containing reference spectral/height info, including Doppler
      ...
      IF ((*(*info).plotswitch).heightset EQ 0) THEN BEGIN
            ...
            VALUE = STRTRIM(STRING((*(*info).plotaxes).v_dop_ref[(*(*info).dataparams).lp_ref],$
                    FORMAT='(3(F9.2,x))'),2), /DYNAMIC_RESIZE)
...
; Reference image window creation procedure
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_DISPLAYS_REF_TOGGLE', /IGNORE_LAST
...
; Updates temporal spectrum display window plot axes range according to set parameters
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_DISPLAYS_SP_REPLOT_AXES'
...
; Temporal spectrum display window creation procedure
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_DISPLAYS_SP_TOGGLE', /IGNORE_LAST
  ; If window doesn't exist, create it
...
; Handles determining warping tie points
  ...
  ELSE $
    ...
    xo = ((lps_sel-min_lps) / FLOAT(MAX(lps_sel-min_lps)) * $
  ...
  IF ((*(*info).dispswitch).warpspslice AND (*(*info).winswitch).showphis) THEN BEGIN
    ...
    CRISPEX_DISPRANGE_GET_WARP_TRIANGULATE, event, $
      ...
      *(*(*info).dispparams).phisxi,*(*(*info).dispparams).phisyi, $
      (*(*(*info).data).phislice)[(*(*info).dispparams).lp_low:(*(*info).dispparams).lp_upp,*], $
      /PHIS, NY=(*(*info).phiparams).nphi
  ENDIF
	IF ~KEYWORD_SET(NO_DRAW) THEN BEGIN
		IF (*(*info).winswitch).showsp THEN CRISPEX_DISPLAYS_SP_REPLOT_AXES, event
		CRISPEX_UPDATE_T, event
...
; Determines range from change in lower or upper s-value and calls (re)display routine
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_DISPRANGE_LP_REF_RANGE'
...
; Handles overplotting of loop paths from the restored and retrieved loops as well as from the retrieved detections
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_DRAW_LOOP_OVERLAYS'
...
;	ENDIF 
  IF ((*(*info).scaling).gamma[scale_idx] NE 1.) THEN BEGIN
    ...
    ENDELSE
  ENDIF
;    minmax_data = (TEMPORARY(minmax_data))^(*(*info).scaling).gamma[scale_idx]
;  minimum = MIN(minmax_data, MAX=maximum)
  IF ((*(*(*info).scaling).imagescale)[sel] EQ 2) THEN $
    minimum = MIN(selected_data, MAX=maximum)
  minmax = CRISPEX_SCALING_CONTRAST(minimum,maximum,$
    (*(*info).scaling).minimum[scale_idx],(*(*info).scaling).maximum[scale_idx])
...
;  GAMMA_CT, 1
  ...
  IF ((*(*info).intparams).nrefdiagnostics GT 1) THEN BEGIN
    ...
    FOR d=0,(*(*info).intparams).ndisp_refdiagnostics-1 DO BEGIN
      sel_idx = (*(*info).intparams).ndiagnostics+wheredispdiag[d]
;      IF (d EQ 0) THEN $
...
;    dispslice = tmp_disp
    ; Display slice
;      tmp_disp *= (*(*info).scaling).mult_val[sel_idx]
...
;      CRISPEX_DRAW_SCALING,event,fin_disp,minimum,maximum, /MAIN, SELECTED_DATA=tmp_disp
      IF ((*(*info).scaling).gamma[sel_idx] NE 1) THEN $
        tmp_disp = (TEMPORARY(tmp_disp))^(*(*info).scaling).gamma[sel_idx]
...
;    dispslice *= (*(*info).scaling).mult_val[sel_idx]
    IF ((*(*info).scaling).gamma[sel_idx] NE 1) THEN $
      dispslice = (TEMPORARY(dispslice))^(*(*info).scaling).gamma[sel_idx]
;    GAMMA_CT,(*(*info).scaling).gamma[(*(*info).intparams).lp_ref_diag_all+$
;      (*(*info).intparams).ndiagnostics]
    refmin = MIN(dispslice,MAX=refmax)
    refminmax = CRISPEX_SCALING_CONTRAST(refmin,refmax,$
      (*(*info).scaling).minimum[sel_idx],(*(*info).scaling).maximum[sel_idx])
...
;  GAMMA_CT, 1
  ...
  IF ((*(*info).intparams).nrefdiagnostics GT 1) THEN BEGIN
    ...
    lp_ref_disprange = (*(*info).dataparams).reflps[diag_starts[lp_ref_diag]+$
                    diag_widths[lp_ref_diag]-1] - $
                    (*(*info).dataparams).reflps[diag_starts[lp_ref_diag]]
  ENDIF ELSE BEGIN
...
; (Re)draw intensity versus time plot procedure
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_DRAW_INT'
...
; (Re)draw detailed spectrum procedure
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_DRAW_LS'
  ...
  ; Loop over all Stokes parameters for detailed spectrum plots
    ...
    FOR d=0,(*(*info).intparams).ndisp_diagnostics-1 DO BEGIN
      ...
      lsx1 = diag_range[d] + lsx0
   		PLOT, (*(*info).dataparams).lps, spec*(*(*info).scaling).mult_val[disp_idx], $
        /NORM, CHARSIZE=1, YS=1, YR=[ls_low_y,ls_upp_y], $
             XR=xrange, YTICKNAME=ytickname, XMINOR=xminor, XTICKINTERVAL=xtickinterval, $
             XTICKNAME=xtickname, XSTYLE = (*(*info).plotswitch).v_dop_set * 8 + 1, $
             XTICK_GET=xtickvals, BACKGROUND = (*(*info).plotparams).bgplotcol, $
             XTITLE = xtitle, YTITLE=ytitle, $
      ...
      IF ((*(*info).scaling).mult_val[disp_idx] NE 1) THEN BEGIN
        XYOUTS,0.1*(xrange[1]-xrange[0])+xrange[0], 0.9*(ls_upp_y-ls_low_y)+ls_low_y, $
          ...
          STRTRIM(FLOOR(ALOG10(ABS((*(*info).scaling).mult_val[disp_idx])))+3+$
          ((*(*info).scaling).mult_val[disp_idx] LT 0),2)+'.1)')+'x',$
          COLOR=(*(*info).plotparams).plotcol, /DATA
      ENDIF
      ; In case of multiple diagnostics
      IF ((*(*info).intparams).ndiagnostics GT 1) THEN BEGIN
        ...
        ENDFOR
        ; Draw lower x-axis for each diagnostic
      	AXIS, XAXIS=0, XTICKLEN = (*(*info).plotaxes).lsxticklen, XRANGE = xrange, XSTYLE=1, $
          COLOR = (*(*info).plotparams).plotcol, XTICKNAME=final_xticks,XTICKINTERVAL=xtickinterval, $
          XMINOR=xminor, XTITLE=xtitle
        ; Draw x-title centered on plot box on first pass
        IF (d EQ 0) THEN $
          XYOUTS,((*(*info).plotpos).lsx1-(*(*info).plotpos).lsx0)/2.+(*(*info).plotpos).lsx0,(*(*info).plotpos).lsy0/3.,$
            (*(*info).plottitles).spxtitle,ALIGNMENT=0.5, COLOR = (*(*info).plotparams).plotcol,/NORMAL
      ENDIF
      ...
      ; Display Doppler velocities on top axis if info available
        ...
        ENDIF ELSE xtickintdop = ''
        ; Draw top axis
  	  	AXIS, XAXIS=1, XTICKLEN = (*(*info).plotaxes).lsxticklen, $;XTICKNAME = xtickname, $
          XRANGE = [((*(*info).plotaxes).v_dop)[(*(*info).dispparams).lp_low], $
                    ((*(*info).plotaxes).v_dop)[(*(*info).dispparams).lp_upp]], XSTYLE=1, $
  	  		XTITLE = topxtitle, COLOR = (*(*info).plotparams).plotcol,XTICKINTERVAL=xtickintdop
      ENDIF 
      ...
      ; Overplot detailed spectrum
        ...
        ssp *= ((*(*info).paramparams).scale_cubes)[0] / (10.^(order_corr))
  		OPLOT, (*(*info).dataparams).lps, ssp*(*(*info).scaling).mult_val[disp_idx], $
        LINE=0, COLOR = (*(*info).plotparams).plotcol
      ; Overplot average minus detailed spectrum
  		IF (*(*info).plotswitch).subtract THEN $
  			OPLOT, (*(*info).dataparams).lps, (spec-ssp)*(*(*info).scaling).mult_val[disp_idx], $
          COLOR = (*(*info).plotparams).plotcol, LINE=2
      ; Overplot spectral indicator
	...
	ENDFOR
	IF (((*(*info).feedbparams).verbosity)[3] EQ 1) THEN $
    CRISPEX_VERBOSE_GET, event, [(*(*info).winids).lswid,ls_low_y,ls_upp_y], $
                        labels=['Window ID for draw','Lower y-value','Upper y-value']
END
...
;    dispslice = tmp_disp
    ; Display slice
;      tmp_disp *= (*(*info).scaling).mult_val[wheredispdiag[d]]
;      GAMMA_CT, (*(*info).scaling).gamma[wheredispdiag[d]]
      IF ((*(*info).scaling).gamma[wheredispdiag[d]] NE 1) THEN $
        tmp_disp = (TEMPORARY(tmp_disp))^(*(*info).scaling).gamma[wheredispdiag[d]]
      minimum = MIN(tmp_disp,MAX=maximum)
      minmax = CRISPEX_SCALING_CONTRAST(minimum,maximum,$
        (*(*info).scaling).minimum[wheredispdiag[d]],(*(*info).scaling).maximum[wheredispdiag[d]])
  ...
  ENDIF ELSE BEGIN
    diag_starts = 0
    ; Display slice
;    dispslice *= (*(*info).scaling).mult_val[(*(*info).intparams).lp_diag_all]
;    GAMMA_CT, (*(*info).scaling).gamma[(*(*info).intparams).lp_diag_all]
    IF ((*(*info).scaling).gamma[(*(*info).intparams).lp_diag_all] NE 1) THEN $
      dispslice = (TEMPORARY(dispslice))^(*(*info).scaling).gamma[(*(*info).intparams).lp_diag_all]
...
;  GAMMA_CT, 1
  ...
  IF ((*(*info).intparams).ndiagnostics GT 1) THEN BEGIN
    ; Determine in which diagnostic the current LP lies
    IF (lp_diag EQ 0) THEN offset = 0 ELSE offset = TOTAL(diag_range[0:(lp_diag-1)])
    spx0 = (*(*info).plotpos).spx0 + offset
    xplspw = diag_range[lp_diag]
    ; Get the corresponding lp_disprange
    lp_disprange = (*(*info).dataparams).lps[diag_starts[lp_diag]+$
                    diag_widths[lp_diag]-1] - $
                    (*(*info).dataparams).lps[diag_starts[lp_diag]]
  ENDIF ELSE BEGIN
...
; (Re)draw spectral slice along a slit procedure
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_DRAW_PHIS'
...
;      GAMMA_CT, (*(*info).scaling).gamma[(*(*info).intparams).lp_diag_all]
      IF ((*(*info).scaling).gamma[(*(*info).intparams).lp_diag_all] NE 1) THEN $
        dispslice = (TEMPORARY(dispslice))^(*(*info).scaling).gamma[$
...
;    GAMMA_CT, 1
    ...
    ENDELSE
  	PLOTS, [1,1] * ( ((*(*info).dataparams).lps[(*(*info).dataparams).lp] - $
                      (*(*info).dataparams).lps[diag_starts[lp_diag]]) / $
                      lp_disprange * phisxplspw + phisx0 ), $
...
;    (*(*info).intparams).ndiagnostics]
  minmax = CRISPEX_SCALING_CONTRAST(minimum,maximum,$
    (*(*info).scaling).minimum[(*(*info).intparams).lp_ref_diag_all+$
    (*(*info).intparams).ndiagnostics], FLOAT($
    (*(*info).scaling).maximum[(*(*info).intparams).lp_ref_diag_all+$
    (*(*info).intparams).ndiagnostics]))
...
; Handles interrupting at runtime
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_INTERRUPT', /IGNORE_LAST
...
; Handles the change of arcseconds per pixel resolution
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_MEASURE_ARCSEC', /IGNORE_LAST
...
; Enables the spatial measurement tool
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_MEASURE_ENABLE', /IGNORE_LAST
...
; Computes the actual distance from the measurement line length and resolution
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_MEASURE_CALC'
  au = 149697870. 
...
; Gets the spectral phi slit slice
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_PHISLIT_GET_SLICE'
	x_pts = COS(!DTOR * (*(*info).phiparams).angle) * (FINDGEN( 2*(*(*info).phiparams).nphi_set/2) - $
              (*(*info).phiparams).nphi_set/2) + FIX((*(*info).dataparams).x)
	y_pts = SIN(!DTOR * (*(*info).phiparams).angle) * (FINDGEN( 2*(*(*info).phiparams).nphi_set/2) - $
              (*(*info).phiparams).nphi_set/2) + FIX((*(*info).dataparams).y)
...
; Handles the selection of scaling 
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SCALING_SELECT_TYPE'
...
PRO CRISPEX_SCALING_APPLY_SELECTED, event
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SCALING_APPLY_SELECTED'
  ; Main image
  IF ((*(*(*info).scaling).imagescale)[0] EQ 0) THEN BEGIN
    minmax_data = (*(*(*info).data).imagedata)[$
      (*(*info).dataparams).s * (*(*info).dataparams).nlp + (*(*info).dataparams).lp]
    minmax_data = (TEMPORARY(minmax_data))^(*(*info).scaling).gamma[$
      (*(*info).intparams).lp_diag_all]
  ...
  IF (*(*info).winswitch).showref THEN BEGIN
    IF ((*(*(*info).scaling).imagescale)[1] EQ 0) THEN BEGIN
      minmax_data = (*(*(*info).data).refdata)[(*(*info).dataparams).lp_ref]
      minmax_data = (TEMPORARY(minmax_data))^(*(*info).scaling).gamma[$
        (*(*info).intparams).ndiagnostics + (*(*info).intparams).lp_ref_diag_all]
  ...
  ; Doppler image
  IF (*(*info).winswitch).showdop THEN BEGIN
    gamma_val = (*(*info).scaling).gamma[(*(*info).intparams).ndiagnostics + $
      (*(*info).intparams).nrefdiagnostics + (*(*info).intparams).lp_diag_all]
  ...
  IF (*(*info).winswitch).showsji THEN BEGIN
    IF ((*(*(*info).scaling).imagescale)[3] EQ 0) THEN BEGIN
      minmax_data = (*(*(*info).data).sjidata)[0]
      minmax_data = (TEMPORARY(minmax_data))^(*(*info).scaling).gamma[$
        2*(*(*info).intparams).ndiagnostics + (*(*info).intparams).nrefdiagnostics]
...
; Handles events from the minimum scaling value slider
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SCALING_SLIDER_MIN'
...
; Handles events from the minimum scaling value slider
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SCALING_SLIDER_MAX'
  idx = (*(*info).scaling).idx
...
; Handles the redrawing of window contents after adjustment of scaling
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SCALING_REDRAW'
...
; Handles the selection of diagnostic for multiplying detailed spectrum
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SCALING_MULTIPLY_LS_SELECT'
...
; Handles the selection of diagnostic for multiplying detailed spectrum
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SCALING_MULTIPLY_LS_VALUE'
...
; Gets the filename for the session save routine
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SESSION_SAVE_WINDOW'
	CRISPEX_SAVE_CHECK_PATH_WRITE, event
...
; Checks the session save filename for validity and overwrite problems
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SESSION_SAVE_CONTINUE'
...
; Handles the overwriting and activates the subsequent saving of the session
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SESSION_SAVE_OVER_CONTINUE'
...
; Handles the actual saving of the session
	...
	WIDGET_CONTROL, /HOURGLASS
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SESSION_SAVE'
...
; Handles the actual restoration of the session
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SESSION_RESTORE_READ_POINTER'
...
PRO CRISPEX_SLIDER_LP_REF_LOCK, event, UNLOCK=unlock
; Handles (un)locking the reference reference to (from) the main spectral position slider
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SLIDER_LP_REF_LOCK'
...
; Handles the the update after change in the spectral position slider
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_SLIDER_LP_UPDATE'
...
; Handles the update of displayed data after change in spectral position
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_UPDATE_LP'
	IF (*(*info).overlayswitch).loopslit AND ((*(*info).loopparams).np GT 0) THEN BEGIN
...
; Decreases the zoomfactor and calls CRISPEX_BGROUP_ZOOMFAC_SET
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_ZOOMFAC_DECR'
...
; Increases the zoomfactor and calls CRISPEX_BGROUP_ZOOMFAC_SET
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_ZOOMFAC_INCR'
  old_factor_idx = WHERE((*(*info).zooming).factorswitch EQ 1)
...
; Handles the change in measurement position after a zoom event
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_ZOOM_MEAS'
;	IF ((*(*info).zooming).factor NE 1) THEN BEGIN
...
; Handles the change in loop positions after a zoom event
	WIDGET_CONTROL, event.TOP, GET_UVALUE = info
	IF (TOTAL(((*(*info).feedbparams).verbosity)[2:3]) GE 1) THEN $
    CRISPEX_VERBOSE_GET_ROUTINE, event, 'CRISPEX_ZOOM_LOOP'
;	IF ((*(*info).zooming).factor NE 1) THEN BEGIN
		*(*(*info).overlayparams).sxr = (*(*(*info).loopparams).xr - (*(*info).zooming).xpos) * $
                                    (*(*info).winsizes).xywinx / ((*(*info).dataparams).d_nx+1)
		*(*(*info).overlayparams).syr = (*(*(*info).loopparams).yr - (*(*info).zooming).ypos) * $
  ...
  dopplerscan = FLTARR(hdr.nx,hdr.ny,hdr.nlp*hdr.ns)
	...
	FOR j=0,hdr.ns-1 DO BEGIN
        ...
        dopplerscan[*,*,j*hdr.nlp+k] = dopplerim
  			dopplermin[k,j] = MIN(dopplerim, MAX=max_val)
...
;	scan	= PTR_NEW(hdr.scanfile, /NO_COPY)
  ...
  sspscan = PTR_NEW(MAKE_ARRAY(hdr.nx,hdr.ny,hdr.nlp*hdr.ns, TYPE=hdr.imtype))
	...
	IF ((hdr.spfile EQ 1) OR (hdr.single_cube[0] GE 1)) THEN BEGIN
		...
		refloopslice = PTR_NEW(BYTARR(nphi,hdr.nt))
		crossloc= PTR_NEW(INTARR(nphi))
		exact_loopslab= PTR_NEW(FLTARR(hdr.nlp,hdr.nt,nphi))
		...
		ENDFOR
		det_loopslab= PTR_NEW(FLTARR(hdr.nlp,hdr.nt,nphi))
